...
For local development, AgentCore permission is not required. Run the Python voice relay locally with temporary AWS credentials that can invoke Nova 2 Sonic, and configure the frontend with the shared Lambda Function URLs in ignored `frontend/.env.local`. See `guides/frontend-backend-wiring.md` for the complete workflow. The eventual hosted deployment remains a separate, single-account Amplify + AgentCore architecture.
