...
That target is not yet the implemented end-to-end state. The current frontend still uses a development mock, hard-codes `ws://localhost:8080` for the real client, and speaks a different WebSocket message format from the relay. Amplify Hosting and browser authentication are also not defined in the repository yet. Maintained guides must label these items as planned until their code and infrastructure exist.
