...
├── test/
│   └── infra.test.ts       # Jest-based infrastructure tests
...
The CDK stack references each Lambda's source code relative to the `infra/` folder:

```
lambda.Code.fromAsset('../analyst')       →  /analyst/
...
|--------|-------------|---------|
| Analyst | `analyst.handler.lambda_handler` | `analyst/handler.py` → function `lambda_handler` |
...
The Interviewer Lambda receives `CONFIG_BUCKET` (the S3 bucket name) as an environment variable. Other Lambdas don't need env vars — they use hardcoded model IDs and region.
...
| Change | Effect |
|--------|--------|
| Edit Python code in a Lambda folder (e.g., `analyst/*.py`) | CDK detects the asset hash changed → redeploys that Lambda |
...
| Handler not found on Lambda invocation | Verify the handler path matches the actual file. E.g., `analyst.handler.lambda_handler` means `analyst/handler.py` contains `def lambda_handler(event, context)` |
