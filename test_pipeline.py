...
avoid import conflicts (analyst/interviewer use bare imports).
...
def main():
    ...
    analyst_output = run_stage("_test_stage_analyst.py", "Analyst")
    ...
    with open(os.path.join(ROOT, "_test_analyst_output.json"), "w") as f:
        json.dump(analyst_output, f)

    interviewer_output = run_stage("_test_stage_interviewer.py", "Interviewer")
    ...
    evaluator_output = run_stage("_test_stage_evaluator.py", "Evaluator")
...
if __name__ == "__main__":
    ...
    # Cleanup temp files
    for f in ["_test_analyst_output.json"]:
