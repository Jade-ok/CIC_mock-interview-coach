...
- **US-1.2** As a candidate, I want to paste a target job description, so that the interview is tailored to that specific role.
- **US-1.3** As a candidate, I want the client and PDF Parser to enforce one consistent 4 MiB (4,194,304 bytes) résumé limit, so that I get instant feedback instead of a failed upload.
...
- **US-4.2** As a candidate, I want to be asked about project ownership, technical problem-solving, and collaboration/role fit, so that I practice the three areas the interview covers.
...
- **US-5.2** As a candidate, I want scores for concrete examples, STAR structure, link to the job, and quantifiable outcomes, so that I know exactly what to work on.
- **US-5.3** As a candidate, I want question-by-question strength and improvement feedback plus overall strengths and improvements, so that I get concrete, actionable takeaways.
