...
Before hosted processing, Demo Session atomically reserves a daily slot and issues a two-hour opaque token bound to the trusted viewer address. Defaults are 100 interviews globally and 5 per viewer IP per UTC day, with bounded attempts for every downstream stage and an edge path/method allowlist. The no-login hosted design also uses exact browser origins, one 55-second model attempt, invocation/error/throttle alarms, an email-backed AWS cost budget, hosted text/output/session limits, and an emergency function switch. Optional normal concurrency caps default off until the target account quota supports them. Pure local mode bypasses hosted admission and leaves the hosted timeout/retry/workload limits disabled while retaining AWS account quotas and the shared 4 MiB PDF limit.
