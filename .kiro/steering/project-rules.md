...
- A Signing Lambda generates a presigned WebSocket URL for Nova Sonic. The frontend connects directly to Nova Sonic using this URL.
...
- The frontend connects directly to Nova Sonic via a presigned WebSocket URL (generated by the Signing Lambda). No proxy server, no containers, no AgentCore.
- Lambda Function URLs are used for the Interviewer, Signing, Analyst, Evaluator, pdf_parser, and Polly endpoints.
...
signing/
  __init__.py
  handler.py          # generates presigned WSS URL for Nova Sonic
```
